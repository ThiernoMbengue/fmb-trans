...
const COLORS = {
  ink: "var(--text-ink)", fleet: "var(--fleet)", amber: "var(--amber)", green: "var(--green)",
  red: "var(--red)", slate: "var(--text-slate)", line: "var(--border-line)",
};
...
function KpiCard({ icon: Icon, label, value, sub, accent }) {
  return (
    <div className="relative bg-[var(--bg-surface)] rounded-xl border border-[var(--border-line)] p-4 flex flex-col gap-2 overflow-hidden">
      <div className="absolute left-0 top-0 h-full w-1" style={{ backgroundColor: accent }} />
      <div className="flex items-center justify-between pl-2">
        <span className="text-xs font-medium tracking-wide uppercase text-[var(--text-slate)]">{label}</span>
      ...
      {sub && <div className="pl-2 text-xs text-[var(--text-slate-light)]">{sub}</div>}
    </div>
  );
}
...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-line)] rounded-lg shadow-md px-3 py-2 text-xs font-mono">
      <div className="text-[var(--text-slate)] mb-1">{label}</div>
      {payload.map((p) => (
        <div key={p.dataKey} className="flex items-center gap-2">
          <span className="inline-block w-2 h-2 rounded-full" style={{ backgroundColor: p.color }} />
          <span className="text-[var(--text-slate)]">{p.name}</span>
  ...
  return (
    <main className="px-6 md:px-10 py-8 max-w-5xl mx-auto">
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="relative">
          <select
            ...
            className="appearance-none text-sm font-medium bg-[var(--bg-surface)] border border-[var(--border-line)] rounded-lg pl-3 pr-8 py-2 cursor-pointer"
          ...
          <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--text-slate-light)]" />
        </div>
        {vehicle && (
        ...
        {months.length > 0 && (
          <div className="relative ml-auto">
            <select
              ...
              className="appearance-none text-sm font-medium bg-[var(--bg-surface)] border border-[var(--border-line)] rounded-lg pl-3 pr-8 py-2 cursor-pointer capitalize"
              style={{ color: COLORS.ink }}
            >
      ...
      {loading ? (
        ...
        <div className="bg-[var(--bg-surface)] rounded-xl border border-[var(--border-line)] p-8 text-center text-sm text-[var(--text-slate-light)]">
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-[var(--bg-surface)] rounded-xl border border-[var(--border-line)] p-5">
            ...
            <div className="bg-[var(--bg-surface)] rounded-xl border border-[var(--border-line)] p-5 flex flex-col">
              ...
              <div className="flex flex-col gap-1.5 mt-2">
                {pieData.map((d) => (
                  <div key={d.name} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-[var(--text-slate)]">{d.name}</span>
