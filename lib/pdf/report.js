...
export function generateRecuPDF(vehicle, decompte, paiements) {
  ...
  y = drawInfoTable(doc, y, [
    ...
    ["Période couverte", decompte.periodeLabel],
  ...
  const lines = doc.splitTextToSize(texte, doc.internal.pageSize.getWidth() - 28);
  doc.text(lines, 14, y);
  y += lines.length * 4.5 + 14;
  ...
  drawFooter(
    doc,
    `Reçu ${numero} — ${vehicle.marque} (${vehicle.immatriculation}), période ${decompte.periodeLabel}. Document généré automatiquement à partir des saisies d'exploitation.`
  );
  return doc;
}
